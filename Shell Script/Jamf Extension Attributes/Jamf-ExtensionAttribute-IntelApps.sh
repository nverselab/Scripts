...
## SOURCE: https://github.com/rtrouton/rtrouton_scripts/blob/main/rtrouton_scripts/detect_installed_intel_based_apps/detect_installed_intel_based_apps.sh

# Detect all Intel apps installed in /Applications, /Library
# or /usr/local and output list Jamf Pro.
...
if [[ "$(/usr/bin/id -u)" -eq 0 ]]; then
	...
	if [[ -n "$intel_app_list" ]]; then
		...
		if [[ -n "$intel_app_list" ]]; then
			echo "<result>$intel_app_list</result>"
		else
		    echo "No Intel applications found" > "$intel_app_logfile"
		fi
	fi
